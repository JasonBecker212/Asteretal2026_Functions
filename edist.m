function [azi,bazi,range,angle] = edist(slat,slon,xlat,xlon)
% function [azi,bazi,range,angle] = edist(slat,slon,xlat,xlon)
% calculates great circle azimuth (degrees), backazimuth (degrees),
% range (km), and angular distance (degrees) between two points
% zero range situation defaults
if isequal(slat, xlat) && isequal(slon, xlon)
    azi=0;
    bazi=0;
    range=0;
    angle=0;
...
%
% edist - distance calculation
% xlat, etc. must be in deg and tenths
% Andoyer Lambert distance formula.
%
